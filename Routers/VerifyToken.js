    ...
    VerfiyToken(request,response,async function(){
        ...
        if(StoredUser){
            if(StoredUser.IsAdmin === false){
                ...
                response.status(401).json('Sorry ! Admin Cannot Make Orders')
            }
        }
        else{
            return response.status(401).json('Sorry ! You Are Not Authenticated')
...
function VerfiySeller(request,response,next){
    VerfiyToken(request,response,async function(){
const SToredSeller= await Seller.findById(request.User.id)
        if(SToredSeller){
            next()
        }
        else{
...
module.exports = { VerfiyToken , VerfiyAuthorization,VerfiyAdmin,VerfiySeller,VerfiyUser}